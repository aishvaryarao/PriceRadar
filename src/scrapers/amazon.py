...
class AmazonScraper(BaseScraper):
    """Scraper for Amazon India marketplace."""
    ...
    def __init__(self) -> None:
        ...
        self.base_url: str = AMAZON_BASE_URL
    ...
    async def _init_browser(self) -> None:
        ...
        if self.browser is None:
            ...
            logger.info("Browser initialized for Amazon scraping")
    ...
    async def search(self, query: str, pages: int = 2) -> list[dict[str, Any]]:
        ...
        try:
            for page_num in range(1, pages + 1):
                logger.info(f"Scraping Amazon search: {query}, page {page_num}")
                ...
                for card in product_cards:
                    try:
                        ...
                        is_sponsored = await card.query_selector(".s-promoted-result-element")
                        if is_sponsored:
                            continue
            ...
            logger.info(f"Amazon search complete: {query} - {len(products)} products found")
            return products
    ...
    async def _extract_product_data(self, card: Any) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Original price
            original_price_elem = await card.query_selector(".a-price-strike")
            ...
            # Discount percentage
            discount_elem = await card.query_selector(".a-badge-percentage span")
            ...
            # Rating
            rating_elem = await card.query_selector(".a-icon-star-small span")
            ...
            # Review count
            review_elem = await card.query_selector("span[aria-label*='rating']")
            review_text = await review_elem.get_attribute("aria-label") if review_elem else None
            review_count = None
    ...
    async def scrape_product(self, url: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Get rating
            rating_elem = await page.query_selector(".a-icon-star span")
            rating_text = await rating_elem.text_content() if rating_elem else None
            rating = self._clean_rating_str(rating_text) if rating_text else None
    ...
    def _clean_rating_str(rating_str: str | None) -> float | None:
        ...
        Args:
            rating_str: Rating string (e.g., '4.2 out of 5')
